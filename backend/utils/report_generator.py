"""
PDF Report Generator for FocusMate Sessions
Creates detailed PDF reports with session statistics
"""
...
class ReportGenerator:
    ...
    def generate_combined_report(self, sessions, period, filename):
        """Generate combined PDF report for multiple sessions"""
        doc = SimpleDocTemplate(filename, pagesize=letter)
        story = []
    ...
    def _get_score_status(self, score):
        """Get status text based on focus score"""
        if score >= 90:
            return 'Excellent'
        elif score >= 80:
            return 'Great'
    ...
    def _generate_recommendations(self, session):
        ...
        if session['posture_warnings'] > 3:
            recommendations.append(
                "Remember to maintain good posture. Consider using a chair with proper back support.")
    ...
    def _generate_combined_insights(self, sessions):
        ...
        # Most productive time (placeholder - would need time analysis)
